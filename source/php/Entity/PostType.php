...
class PostType implements PostTypeInterface
{
    ...
    {
        $loweCaseNameSingular = __(strtolower($this->nameSingular), ASM_TEXT_DOMAIN);
        $loweCaseNamePlural   = __(strtolower($this->namePlural), ASM_TEXT_DOMAIN);

        $labels = array_merge(
            array(
                'name'               => __(ucfirst($this->namePlural), ASM_TEXT_DOMAIN),
                'singular_name'      => __(ucfirst($this->nameSingular), ASM_TEXT_DOMAIN),
                'add_new'            => sprintf(__('Add new %s', ASM_TEXT_DOMAIN), $loweCaseNameSingular),
                'add_new_item'       => sprintf(__('Add new %s', ASM_TEXT_DOMAIN), $loweCaseNameSingular),
                'edit_item'          => sprintf(__('Edit %s', ASM_TEXT_DOMAIN), $loweCaseNameSingular),
                'new_item'           => sprintf(__('New %s', ASM_TEXT_DOMAIN), $loweCaseNameSingular),
                'view_item'          => sprintf(__('View %s', ASM_TEXT_DOMAIN), $loweCaseNameSingular),
                'search_items'       => sprintf(__('Search %s', ASM_TEXT_DOMAIN), $loweCaseNamePlural),
                'not_found'          => sprintf(__('No %s found', ASM_TEXT_DOMAIN), $loweCaseNamePlural),
                'not_found_in_trash' => sprintf(__('No %s found in trash', ASM_TEXT_DOMAIN), $loweCaseNamePlural),
                'parent_item_colon'  => sprintf(__('Parent %s:', ASM_TEXT_DOMAIN), $loweCaseNameSingular),
